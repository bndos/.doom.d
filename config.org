...
(after! dap-mode
  ;; Customize pending breakpoints
  (custom-set-faces
   '(dap-ui-pending-breakpoint-face
     ((((background dark)) (:background "#191b1a" :foreground "#ffffff" :weight bold))
      (t (:foreground "#4a6b82" :weight bold))))

   '(dap-ui-verified-breakpoint-face
     ((((background dark)) (:background "#3a3b3a" :foreground "#ffffff" :weight bold))
      (t (:foreground "#4a6b82" :weight bold))))

   '(dap-ui-marker-face
     ((((background dark)) (:background "#ff79c6" :foreground "#000000" :weight bold))
      (t (:background "#ff79c6" :foreground "#000000" :weight bold))))
   )
  ...
  (setq dap-auto-configure-features '(locals controls tooltip breakpoints repl))
  (dap-ui-mode 1)
  (dap-tooltip-mode 1)
  (tooltip-mode 1)
  (dap-ui-controls-mode 1)
  (defun my/dap-ui--breakpoint-visuals (breakpoint breakpoint-dap)
    "Customize visuals for a BREAKPOINT."
    (list :face (if (and breakpoint-dap (gethash "verified" breakpoint-dap))
                    'dap-ui-verified-breakpoint-face
                  'dap-ui-pending-breakpoint-face)
          :char (if (and breakpoint-dap (gethash "verified" breakpoint-dap))
                    "*" ;; Verified breakpoint
                  "x")  ;; Pending breakpoint
          :priority (+ dap-ui-overlay-priority 1)))

  (advice-add 'dap-ui--breakpoint-visuals :override #'my/dap-ui--breakpoint-visuals)
  (defun my/dap-ui--set-debug-marker (file point)
    ...
    (setq-local dap-ui--cursor-overlay
                (dap-ui--make-overlay-at
                 ...
                 (list :face 'dap-ui-marker-face
                       :char "=>"  ;; Current line marker
                       :priority (+ dap-ui-overlay-priority 2)))))
  (advice-add 'dap-ui--set-debug-marker :override #'my/dap-ui--set-debug-marker))
...
(use-package eglot
  ...
  (map! :map eglot-mode-map
        ...
        ;; :leader "c r" #'lsp-proxy-rename
        ;; :leader "c a" #'lsp-proxy-execute-code-action
        ;; :leader "c d" #'lsp-proxy-describe-thing-at-point
        ;; :leader "c x" #'lsp-proxy-show-project-diagnostics
        )
