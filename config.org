...
(setq
 ...
 `(
   ...
   ("^magit:" display-buffer-in-side-window
    ...
    (preserve-size . (t . nil)) ,parameters)
                   ("\\*\\(?:help\\|grep\\|Completions\\|org-python-session\\|Python\\)\\*\\|^*leetcode\\|^*compilation\\|^*Flutter\\|^*pytest\\|^*docker-build-output\\|^*ein\\|^*MATLAB\\|^* docker container"
    (display-buffer-reuse-window display-buffer-in-side-window)
...
(after! org
  ...
  ;; temp fix for org 9.7
  (defun my-org-babel-python--preemptive-init ()
    (setq-local org-babel-python--initialized t))
  
  (add-hook
   'python-shell-first-prompt-hook
   'my-org-babel-python--preemptive-init)
