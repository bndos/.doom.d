...
(dolist (hk '(js-ts-mode-hook typescript-ts-mode-hook tsx-ts-mode-hook rjsx-mode-hook typescript-mode-hook))
  (add-hook hk
            (lambda ()
              ;; Let Doom manage formatting on save (no race conditions)
              (+format-on-save-mode 1)
              ;; Run organize-imports just before save
              )))
...
(defun my/tabspaces--workspace-p (b)
  "Return non-nil if buffer B belongs to the current tabspace."
  (and (buffer-live-p b)
       (not (minibufferp b))
       (let ((bn (buffer-name b)))
         (not (and bn (string-prefix-p " *Minibuf-" bn))))
       (if (fboundp 'tabspaces--local-buffer-p)
           (tabspaces--local-buffer-p b)
         t)))

(defvar my/consult-source-tabspace
  `(:name     "Tabspace Buffers (MRU)"
    :narrow   ?t
    :category buffer
    :state    ,#'consult--buffer-state
    :history  buffer-name-history
    ;; Items are produced from the real MRU `buffer-list`, filtered to tabspace.
    ;; `:sort` is handled by Consult; using `:sort 'visibility` gives
    ;; the nice "recent + currently visible demoted" behavior.
    :items    ,(lambda ()
                 (consult--buffer-query
                  :sort 'visibility
                  :predicate #'my/tabspaces--workspace-p
                  :as #'buffer-name))))

(defun my/consult-buffer-tabspace ()
  "Consult-powered buffer switcher limited to the current tabspace."
  (interactive)
  (consult-buffer (list my/consult-source-tabspace)))

;; Wire it into tabspaces so your existing remap keeps working.
(with-eval-after-load 'tabspaces
  (ignore-errors
    (advice-remove 'tabspaces-switch-to-buffer #'my/consult-buffer-tabspace))
  (advice-add 'tabspaces-switch-to-buffer :override #'my/consult-buffer-tabspace))
...
(defun my/tabspaces--unique-name (base)
  "Return a unique workspace name starting with BASE."
  (let* ((existing (and (fboundp 'tabspaces--list-tabspaces)
                        (tabspaces--list-tabspaces)))
         (i 1)
         (name base))
    (while (and existing (member name existing))
      (setq name (format "%s-%d" base i)
            i (1+ i)))
    name))

(defun my/tabspaces-new ()
  "Create a new tabspace.
If a Projectile project is selected, name it after the project and switch there.
If the user presses RET or Projectile isn't available, make a scratch workspace."
  (interactive)
  (let* ((has-proj (require 'projectile nil t))
         (projects (and has-proj (projectile-relevant-known-projects)))
         (choice (and projects
                      (completing-read
                       "New workspace from project (RET for scratch): "
                       projects nil ;; predicate
                       nil          ;; require-match = nil so RET yields \"\"
                       nil 'projectile-project-history))))
    (cond
     ;; Project chosen → workspace named after project, then switch project
     ((and choice (not (string-empty-p choice)))
      (let* ((proj choice)
             (ws-name (file-name-nondirectory (directory-file-name proj))))
        (my/tabspaces--switch ws-name)
        (projectile-switch-project-by-name proj)))
     ;; No project picked → scratch workspace
     (t
      (let ((ws-name (my/tabspaces--unique-name "scratch")))
        (my/tabspaces--switch ws-name)
