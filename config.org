...
(defun pr-review--collect-comment-positions ()
  ...
  (let (positions)
    (save-excursion
      ...
      (while-let ((m (text-property-search-forward
                      'magit-section nil
                      (lambda (_ v) (cl-typep v 'pr-review--review-thread-section)))))
        (push (prop-match-beginning m) positions)
...
(defun pr-review--comment-skip-pos (pos direction)
  ...
  (let ((section (get-text-property pos 'magit-section)))
    (cond
     ((cl-typep section 'pr-review--review-thread-section)
...
(use-package! vterm
  ...
  :config
  ;; Strip leading spinner chars (e.g. from codex) before vterm renames the
  ;; buffer.  Without this, persp-mode saves the window config under the
  ;; transient spinner name; on restore it can't find that buffer (the spinner
  ;; has advanced) and calls get-buffer-create, producing an empty
  ;; fundamental-mode ghost that occupies the window instead of the real vterm.
  (defun vterm--strip-spinner-title (args)
    (list (replace-regexp-in-string
           (rx bos (any (#x2800 . #x28FF)) " ")
           "" (car args))))
  (advice-add 'vterm--set-title :filter-args #'vterm--strip-spinner-title))
#+END_SRC
